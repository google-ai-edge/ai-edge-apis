...
Java_com_google_ai_edge_localagents_fc_GemmaFormatter_nativeFormatSystemMessage(
  ...
  absl::StatusOr<std::string> prompt =
      odml::generativeai::FormatGemmaSystemMessage(request.system_instruction(),
...
JNIEXPORT jstring JNICALL
Java_com_google_ai_edge_localagents_fc_GemmaFormatter_nativeFormatContent(
  ...
  absl::StatusOr<std::string> prompt =
      odml::generativeai::FormatGemmaContent(content, options);
  if (!prompt.ok()) {
    return nullptr;
  }
  return env->NewStringUTF(prompt.value().c_str());
...
JNIEXPORT jstring JNICALL
Java_com_google_ai_edge_localagents_fc_GemmaFormatter_nativeStartModelTurn(
    JNIEnv* env, jclass ignored, jbyteArray options_bytes) {
  ModelFormatterOptions options;
  if (!ParseProto(env, options_bytes, options)) {
    return nullptr;
  }
  return env->NewStringUTF(odml::generativeai::StartGemmaTurn(options).c_str());
}

JNIEXPORT jstring JNICALL
Java_com_google_ai_edge_localagents_fc_GemmaFormatter_nativeFormatRequest(
    JNIEnv* env, jclass ignored, jbyteArray request_bytes,
    jbyteArray options_bytes) {
  GenerateContentRequest request;
  if (!ParseProto(env, request_bytes, request)) {
  ...
  absl::StatusOr<std::string> prompt =
      odml::generativeai::FormatGemmaRequest(request, options);
  if (!prompt.ok()) {
    return nullptr;
  }
  return env->NewStringUTF(prompt.value().c_str());
...
Java_com_google_ai_edge_localagents_fc_GemmaFormatter_nativeParseResponse(
    JNIEnv* env, jclass ignored, jstring output) {
  auto response = odml::generativeai::ParseGemmaResponse(
