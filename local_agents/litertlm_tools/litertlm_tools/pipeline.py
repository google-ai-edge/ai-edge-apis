...
from litertlm_tools import model_downloader as model_downloader_lib
from litertlm_tools import tokenizer as tokenizer_lib
import numpy as np
...
class LiteRTLlmPipelineLoader(LlmPipelineLoader):
  ...
  def load(
    ...
    if isinstance(
      ...
      # Load tokenizer using AutoTokenizer from the specified location
      # This location could be the same as repo_id or a different one
      logging.info("Loading HF tokenizer from: %s", tokenizer_location)
      try:
        ...
        logging.error(
            "Failed to load HuggingFace tokenizer from %s: %s",
            tokenizer_location,
            e,
        )
        raise
      ...
      raise ValueError(
          "Unsupported model downloader: %s" % type(self.model_downloader)
      )
    # Wrap the loaded tokenizer
    tokenizer = tokenizer_lib.Tokenizer(raw_tokenizer)
