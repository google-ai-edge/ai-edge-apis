...
class ModelDownloader(abc.ABC):
  ...
  def download_file(self, root_location: str, filename: str):
...
class HuggingFaceDownloader(ModelDownloader):
  ...
  def download_file(self, root_location: str, filename: str) -> str | None:
    local_file_path = os.path.join(self.cache_dir, root_location, filename)
    ...
    try:
      ...
      logging.info(
          "%s/%s downloaded and saved to %s", root_location, filename, file_path
      )
      return file_path
