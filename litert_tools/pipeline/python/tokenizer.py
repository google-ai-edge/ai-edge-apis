...
class Tokenizer:
  ...
  def __init__(
      self,
      tokenizer: Union[sp.SentencePieceProcessor, Any],
      prompt_template: prompt_template_lib.PromptTemplate | None = None,
  ...
  def _apply_prompt_template(self, prompt: str) -> Sequence[int]:
    """Encodes the prompt with the appropriate chat/prompt template."""
    token_ids = []
    if self._prompt_template is not None:
      if self._prompt_template.start_token is not None:
        token_ids.append(
            self._tokenizer.PieceToId(self._prompt_template.start_token)
        )
      elif self._prompt_template.start_token_id is not None:
        token_ids.append(self._prompt_template.start_token_id)

      if self._prompt_template.prompt_prefix is not None:
        token_ids.extend(
            self._tokenizer.EncodeAsIds(self._prompt_template.prompt_prefix)
        )
  ...
  def encode_with_prompt_template(self, prompt: str) -> Sequence[int]:
    ...
    if self._is_sentencepiece:
      if self._prompt_template is not None:
        return self._apply_prompt_template(prompt)
      else:
